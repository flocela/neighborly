...
{   
    REQUIRE_THROWS_WITH(HappinessFuncFlat(1.1), Contains("must be between 0.0 and 1.0 inclusive") );
    REQUIRE_THROWS_WITH(HappinessFuncFlat(-0.2), Contains("must be between 0.0 and 1.0 inclusive"));
}


TEST_CASE("HappinessFuncFlat getHappiness() returns correct happiness", "[happinessfunc_flat]")
{
    HappinessFuncFlat funcZero{0};
    HappinessFuncFlat funcQuarter{0.25};
    HappinessFuncFlat funcOne{1};
...
{   
    REQUIRE_THROWS_WITH(HappinessFuncRising(0.0, 1.1), Contains("must be between 0.0 and 1.0 inclusive"));
    REQUIRE_THROWS_WITH(HappinessFuncRising(-0.2, 1.0), Contains("must be between 0.0 and 1.0 inclusive"));
    REQUIRE_THROWS_WITH(HappinessFuncRising(0.5, 0.2), Contains("hapinessAtOneDiversity must be larger than at happinessAtZeroDiversity"));
}

TEST_CASE("HappinessFuncRising getHappiness()", "[happinessfuncrising]")
{
    HappinessFuncRising funcRising{.5, 1.0};
...
{
    REQUIRE_THROWS_WITH(HappinessFuncStepDown(1.1, .5, .5), Contains("must be between 0.0 and 1.0 inclusive"));
    REQUIRE_THROWS_WITH(HappinessFuncStepDown(0.5, 1.0, .5), Contains("happinessAtZeroDiversity must be larger than happinessAtOneDiversity"));
}

TEST_CASE("HappinessFuncStepDown getHappiness()", "[happinessfuncstepdown]")
{
    HappinessFuncStepDown stepDownFunc{0.9, 0.25, 0.25};
